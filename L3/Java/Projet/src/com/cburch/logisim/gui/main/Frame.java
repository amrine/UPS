...
public class Frame extends LFrame implements LocaleListener {
	...
	BuildCircuitButton btnBuildCircuit; // build circuit
	JButton btnShowTruthTable; // show truth table
	TableTab truthTable; // truth table panel

	// gridbag layout for the custom panel on the canvas
	GridBagLayout gbCanvas = new GridBagLayout();
	GridBagConstraints gcCanvas = new GridBagConstraints();
	...
	private void addNewComponents() {
		...
		// Build circuit
		btnBuildCircuit = new BuildCircuitButton(this, model);
		btnBuildCircuit
				.setToolTipText("Build and display the logic circuit on the canvas.");
		btnBuildCircuit.setText("Build circuit");
		...
		// gridbag layout for components on the canvas
		...
		layoutCanvas.setLayout(gbCanvas);
		gcCanvas.anchor = GridBagConstraints.LINE_START;
		gcCanvas.fill = GridBagConstraints.NONE;

		gbCanvas.setConstraints(customPanel, gcCanvas);
		layoutCanvas.add(customPanel);

		truthTable = new TableTab(model.getTruthTable());
		gbCanvas.setConstraints(truthTable, gcCanvas);
		layoutCanvas.add(truthTable);
	...
	private void buildCircuit() {
		...
		l.add("x");
		this.model.setVariables(new ParseText(txtExpression.getText()).getListe(),l);
		try {
			Expression expr = Parser.parse(txtExpression.getText(), this.model);
			this.model.getOutputExpressions().setExpression("x", expr);
