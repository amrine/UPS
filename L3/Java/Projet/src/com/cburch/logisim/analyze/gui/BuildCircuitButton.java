...
public class BuildCircuitButton extends JButton {
		...
		public void actionPerformed(ActionEvent event) {
			...
			while (!ok) {
				...
				ProjectItem projectItem = (ProjectItem) dlog.project
						.getSelectedItem();
				if (projectItem == null) {
					JOptionPane.showMessageDialog(parent, Strings
							.get("buildNeedProjectError"), Strings
							.get("buildDialogErrorTitle"),
				...
				if (name.equals("")) {
					JOptionPane.showMessageDialog(parent, Strings
							.get("buildNeedCircuitError"), Strings
							.get("buildDialogErrorTitle"),
							JOptionPane.ERROR_MESSAGE);
					continue;
				}
				...
				if (dest.getLogisimFile().getCircuit(name) != null) {
					int choice = JOptionPane.showConfirmDialog(parent,
							StringUtil.format(Strings
									.get("buildConfirmReplaceMessage"), name),
	...
	public BuildCircuitButton(JFrame parent, AnalyzerModel model) {
		this.parent = parent;
		this.model = model;
