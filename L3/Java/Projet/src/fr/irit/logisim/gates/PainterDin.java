...
class PainterDin {
	...
	private static void paintOrLines(InstancePainter painter,
		...
		GateAttributes attrs = (GateAttributes) AndGate.FACTORY.createAttributeSet();
		attrs.inputs = inputs;
		...
		if (lens == null) {
			...
			for (int i = 0; i < inputs; i++) {
				int y = AndGate.FACTORY.getInputOffset(attrs, i).getY();
				if (y < 0) y = -y;
				if (y <= yCurveStart) {
					lens[i] = r;
				} else {
		...
		AbstractGate factory = AndGate.FACTORY;
