...
import fr.irit.logisim.gate.attributes.GateAttributes;
...
public class PainterShaped {
	...
	static Location getInputOffset(GateAttributes attrs, int index) {
		int inputs = attrs.inputs;
		Direction facing = attrs.facing;
		int size = ((Integer) attrs.size.getValue()).intValue();
		int axisLength = size + 0 + (false ? 10 : 0);
		int negated = attrs.negated;
	...
	private static int[] getInputLineLengths(GateAttributes attrs,
		...
		Location loc0 = getInputOffset(attrs, 0);
		Location locn = getInputOffset(attrs, inputs - 1);
		...
		for (int i = 0; i < inputs; i++) {
			Location loci = getInputOffset(attrs, i);
